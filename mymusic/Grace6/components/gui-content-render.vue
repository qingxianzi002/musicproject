<template>
	...
	:style="{
		...
		:key="index">
			...
			:user-select="true" 
			:decode="true" 
			v-else-if="item.type == 'center'">{{item.content}}</text>
			<!-- 图片 -->
			<view 
			class="gui-img-in" 
			v-else-if="item.type == 'img' || item.type == 'image'" 
			...
			|| item.type == 'quote' 
			|| item.type == 'pre'">{{item.content}}</text>
			...
			<!-- 链接 -->
			<view 
			v-else-if="item.type == 'link' || item.type == 'a'">
				<gui-a :link="item.content"></gui-a>
			</view>
